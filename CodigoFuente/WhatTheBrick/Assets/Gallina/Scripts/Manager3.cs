...
public class Manager3 : MonoBehaviour
{
    ...
    private float tiempoPasado_Gallina = 0f;
    private bool moverIzquierda = false; 
    private bool movGallina = false;
    ...
    void Update()
    {
        ...
        tiempoPasado_Gallina += Time.deltaTime;
        int cestasLlenas = 0;
        ...
        if(tiempoPasado_Gallina > 5f){
            movGallina = !movGallina;
            tiempoPasado_Gallina = 0f;
            Gallina.transform.localScale = new Vector3(-1 * Gallina.transform.localScale.x, Gallina.transform.localScale.y, Gallina.transform.localScale.z);
        }

        if(movGallina == false)
            Gallina.transform.Translate(Vector3.right * 2f * Time.deltaTime);
        else
            Gallina.transform.Translate(Vector3.left * 2f *Time.deltaTime);
